...
source("score.R")
source("processLabelBySVM_linear_threshold.R")

run.name <- "svm_linear_threshold"
threshold <- 0.3
...
label.block.size <- 32 # Number of training fits to run in parallel without feed back.
...
for (label.block.start in label.seq) {
    ...
    label.subResults <- foreach (label=label.subSeq, .combine=rbind, .packages=packages) %dopar% {
        set.seed(42)
        label.elapse.seconds <- system.time(results <- processLabelBySVM_linear_threshold(logUNC, label, trainPercent, y.train.all.labels.string, X.train.all, X.test, threshold))[3]
