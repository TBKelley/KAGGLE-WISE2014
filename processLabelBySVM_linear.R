...
processLabelBySVM_linear <- function(logUNC, label, trainPercent, y.train.all.labels.string, X.train.all, X.test) {
    model.name <- "smv_linear"
    model.elapse.seconds <- 0
    pid <- Sys.getpid()
    
    sink(logUNC, append=TRUE)
    cat(sprintf("%s %s(%d): label=%d - Start\n", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), model.name, pid, label))
    sink()
    ...
    if (y.train.int.sum == 0) { # All training = FALSE
        ...
        model.elapse.seconds <- system.time(smv.model <- svm(x=X.train, y=y.train.bool.factor, scale=FALSE, kernel="linear", class.weights=weights, probability=TRUE, seed=42))[3]
